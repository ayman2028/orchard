...
After reading the requirements, I first broke down the problem into functional requirements, which were provided, and non-functional requirements as I will list.
...
Then I created the URL pettern for this api call, by creating scheduling/url.py and creating the path for the endpoint. Used AI to also speed this process up, however I gave it the exact pettern I wanted.

Next I had to set up the API View, FastAvailableTimeslotsAPIView, I am familiar most with the APIView class to set up HTTP returns so I used that to create classes in scheduling/views.py. I first just wanted to verify everything up until this point so I just set up the class and had AI fill out simple return information to process a start-date, end_date returst and just return a bunch of random dates if it was valid or the appropriate HTTP response codes otherwise.
I also had AI create a simple script that would call this API and and print out the raw JSON data so I can quickly verify everything was working fine up until now.
...
returns the exact information I needed. This worked well too. NOTE: This was just to test, I know that it is innefficient. 

Algorithm design:
I was ready to start the algorthm for this API call, For this I didn't just let AI create it I first designed my own process since I noticed that AI often does not give the best design or take scalability into account. 
My thought process was to first create 30 minute windows within working hours within the provided working hours. This would be done for agent whcih has not yet met the daily or weekly cap. Then for each of the agents, we can now see if we 
...
For each of these slots, we then go to the list of confirmed appointments and events and filter out the unavailable slots as well as 30 min windows before and after the appointments. We will be left with a list of available appointments for each agent.
...
and had AI fill out the functions with my supervision while writng down what each funcion did. I also caught some bugs such as AI hard coding the caps to 1 daily and 3 weekly instead of using the respective fields in the Database.

Once I filled out this algorithm I was ready to test this out. The first method I used to test this was to have AI quickly write a script for parsing and displaying the raw JSON output of the API. Once I was sattisfied with this I even asked AI to quickly
create a visualization for me by quickly making an UI as the homepage which would call this view class and display the output in a organized manner. So if you go to http://127.0.0.1:8000/ after running the server it will show the available time slots for a specified time.

NOW FOR TESTING:
I asked AI to come up with a list of tests, exhaustive tests and bench mark tests. Once I was satisfied with the list, adding response codes and edgecaes, which are listed in a seperate file. I had AI use the built in Django test suite to 
...
OPTIMIZATIONS:
I noticed that we are creating these available time slots whenever we call the API, this is highly innificent for a read heavy system. One quick solution that I can think of is to cache the list of available time slots for each agent in a
seperate table then use the Django cahce feature to take care of any dirty data when handling any bookings or other write information. However, in a scalable system, if this process was running on many machines, then keeping the cache the same for all of the devices would be a headache without something like redis. 
So a solution I thought of for now which is an inbetween is to create a table for these available time slots and populate it with the same algothim at the beginning of starting the server by modifying ready() and setting up signals to modify it when new information is written to the original database. 

I used the AI to quickly implement this step and create a NEW APIVIEW which I could use to compare to the original called doctor_schedule_view which is the same as the older one just using the optimized method. Then by using the tests I had created I can compare the the improvements that this optiization had made, it had made a improvement of at most 18x. 

Then I used AI to quiuckly add to the next section of the read me which details the results as well as the steps to test this out on your own. 
